...
import { FileText, Plus, Trash2, Download, ChevronDown } from "lucide-react";
...
const MCQGeneratorTool = () => {
  ...
  const exportToPDF = () => {
    ...
    pdf.text('Multiple Choice Questions', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 20;
  ...
  const exportToGoogleForm = () => {
    ...
    navigator.clipboard.writeText(formattedText).then(() => {
      ...
      alert('Questions copied to clipboard! Paste them into your Google Form.');
    }).catch(err => {
    ...
    >
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <button onClick={generateQuestionsFromText} className="btn-primary mt-4">
            Generate Questions
          </button>
        </div>
                ...
                {currentQuestion.options.map((option, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    {currentQuestion.correctAnswer === index && (
                      <span className="text-sm text-primary font-medium">Correct</span>
                    )}
                  </div>
                ))}
              </div>
                      ...
                      <div className="space-y-2">
                        {question.options.map((option, oIndex) => (
                          ...
                          >
                            ...
                            {question.correctAnswer === oIndex && (
                              <span className="text-xs text-primary font-medium ml-auto">✓ Correct</span>
                            )}
                          </div>
                        ))}
                      </div>

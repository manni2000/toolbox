...
// Website Screenshot - Requires cloud service
router.post('/website-screenshot', async (req, res) => {
  return res.status(503).json({
    error: 'Website screenshot requires cloud service',
    message: 'Browser automation is not available in serverless environments due to browser binary size constraints.',
    alternatives: [
      'Use Browserless.io (https://browserless.io/)',
      'Use Puppeteer Cloud (https://cloud.puppeteer.com/)',
      'Use Apify (https://apify.com/)',
      'Use ScreenshotAPI (https://screenshotapi.net/)',
      'Use Urlbox (https://urlbox.io/)'
    ],
    note: 'Browser binaries required for screenshots are too large for Vercel serverless functions (250MB limit). Consider integrating a cloud service for this feature.'
  });
});
